...
# Validates: gh CLI, Docker daemon, AWS proxy (awsproxy ent).
#
# Exit codes:
...
#   ./check-auth.sh --quiet     — print only problems, nothing on success
set -uo pipefail
...
# ── helpers ──────────────────────────────────────────────────────────────────
# `green` and `bold` are the success/progress channel, so `--quiet` drops them
# and keeps only `yellow`/`red`. A passing pre-flight then says nothing at all:
# the whole value of these checks is the reminder when something is *not* set up
# (an expired gh token, a forgotten `awsproxy ent`), and eight green ticks on
# every rebuild is noise that trains you to stop reading them.
green()  { $QUIET || printf '\033[32m  ✓ %s\033[0m\n' "$*"; }
...
# ── 3. AWS proxy (awsproxy ent) ────────────────────────────────────────────────
bold "Checking AWS proxy..."

# awsproxy is a shell function (sourced from .zshrc), not a binary — it cannot
# be called from a script.  We detect whether it's already been activated in
# the parent shell by checking the proxy env vars it sets.
#
# Any proxy setting counts as activation by default.  Set SIDECAR_PROXY_PATTERN
# to a regex matching your organization's proxy host if you need the check to be
# specific — the hostname is site-local, so it is not hardcoded here.
_aws_proxy_active() {
  local proxy="${HTTPS_PROXY:-}${https_proxy:-}${HTTP_PROXY:-}${http_proxy:-}"
  [[ -n "$proxy" && "$proxy" =~ ${SIDECAR_PROXY_PATTERN:-.} ]]
}

if ! command -v aws &>/dev/null; then
  yellow "aws CLI not found — skipping AWS proxy check"
elif _aws_proxy_active; then
  if aws sts get-caller-identity --output text &>/dev/null; then
    ...
    green "AWS proxy active, credentials valid ($IDENTITY)"
  else
    yellow "AWS proxy env var set but credentials still not valid — try: awsproxy ent"
  fi
else
  yellow "AWS proxy not active — run 'awsproxy ent' in your shell before starting the sidecar"
  printf '  (awsproxy is a shell function and must be run in your terminal, not this script)\n'
fi

# ── Summary ───────────────────────────────────────────────────────────────────
# The blank separator belongs with the output it separates, so it is suppressed
# along with everything else when a quiet run has nothing to report.
if [[ $FAILED -eq 0 ]]; then
  $QUIET || printf '\n'
